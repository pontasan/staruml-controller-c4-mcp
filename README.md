# StarUML Controller MCP Server — C4 Model Diagram

An MCP (Model Context Protocol) server specialized for **C4 Model Diagrams**. Enables AI assistants like Claude to programmatically create and manage C4 model diagrams — persons, software systems, containers, components, and relationships in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="Healthcare Platform — C4 Architecture diagram created entirely via MCP" width="800">
</p>

<p align="center"><em>Healthcare Platform C4 Architecture — created entirely by Claude via this MCP server</em></p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-c4-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-c4-mcp.git
cd staruml-controller-c4-mcp
npm install && npm run build
```
...
5. All C4 Model Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-c4": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-c4-mcp/dist/index.js"],
...
```bash
claude mcp add staruml-c4 -e STARUML_PASSWORD=<password> -- node /absolute/path/to/staruml-controller-c4-mcp/dist/index.js
```
...
### C4 Model Diagram Tools

The core of this server — full CRUD for every C4 Model Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `c4_list_diagrams`, `c4_create_diagram`, `c4_get_diagram`, `c4_update_diagram`, `c4_delete_diagram` |
| **Elements** | `c4_list_elements`, `c4_create_element`, `c4_get_element`, `c4_update_element`, `c4_delete_element` |
| **Relationships** | `c4_list_relationships`, `c4_create_relationship`, `c4_get_relationship`, `c4_update_relationship`, `c4_delete_relationship` |
